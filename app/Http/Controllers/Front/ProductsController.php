...
class ProductsController extends Controller
{
    ...
    { // Required Parameters: https://laravel.com/docs/9.x/routing#required-parameters
        ...
        $similarProducts = Product::with('brand')
            ...
            ->withAvg('ratings', 'rating')->where('category_id', $productDetails['category']['id'])->where('id', '!=', $id)->limit(4)->inRandomOrder()->get()->toArray(); // where('id', '!=', $id)    means get all similar products (of the same category) EXCEPT (exclude) the currently viewed product (to not be repeated (to prevent repetition))    // limit(4)->inRandomOrder()    means show only 4 similar products but IN RANDOM ORDER
        ...
        $recentlyViewedProducts = Product::with('brand')
            ->withCount('ratings') // number of reviews
            ->withAvg('ratings', 'rating')->whereIn('id', $recentProductsIds)->get()->toArray(); // https://laravel.com/docs/9.x/collections#method-wherein AND https://laravel.com/docs/9.x/queries#additional-where-clauses
