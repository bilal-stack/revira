...
class AdminController extends Controller
{
    ...
    public function updateAdminVendorDetails($id, Request $request)
    {
        // Correcting issues in the Skydash Admin Panel Sidebar using Session
        Session::put('page', 'update_personal_details');
        ...
        if ($request->isMethod('post')) {
            ...
            $this->updateVendorPersonalDetails($id, $request);
        ...
        $vendorDetails = Vendor::where('id', $id)->with('vendorbusinessdetails', 'vendorbankdetails')->first()->toArray();
        $image = Admin::where('vendor_id', $vendorDetails['id'])->first()->toArray();
        $image = $image['image'];
        $countries = Country::where('status', 1)->get()->toArray();

        return view('admin/admins/edit_vendor_details')->with(compact('vendorDetails', 'countries', 'image'));
    }

    private function updateVendorPersonalDetails($id, $request)
    {
        $data = $request->all();
        ...
        Admin::where('vendor_id', $id)->update([
            'name' => $data['vendor_name'],
            'mobile' => $data['vendor_mobile'],
            'image' => $imageName,
    ...
    private function updateVendorBusinessDetails($id, $request)
    {
        $data = $request->all();
        // Uploading Admin Photo    // Using the Intervention package for uploading images
        if ($request->hasFile('address_proof_image')) { // the HTML name attribute    name="admin_name"    in update_admin_details.blade.php
        ...
        $vendorCount = VendorsBusinessDetail::where('vendor_id', $id)->count(); // Accessing Specific Guard Instances: https://laravel.com/docs/9.x/authentication#accessing-specific-guard-instances
        if ($vendorCount > 0) { // if there's a vendor already existing, them UPDATE
            // UPDATE `vendors_business_details` table
            VendorsBusinessDetail::where('vendor_id', $id)->update([ // Accessing Specific Guard Instances: https://laravel.com/docs/9.x/authentication#accessing-specific-guard-instances
                'shop_name' => $data['shop_name'],
                'shop_mobile' => $data['shop_mobile'],
                'shop_website' => $data['shop_website'],
                'shop_address' => $data['shop_address'],
            ...
            VendorsBusinessDetail::insert([
                'vendor_id' => $id, // Accessing Specific Guard Instances: https://laravel.com/docs/9.x/authentication#accessing-specific-guard-instances
                'shop_name' => $data['shop_name'],
                'shop_mobile' => $data['shop_mobile'],
                'shop_website' => $data['shop_website'],
                'shop_address' => $data['shop_address'],
    ...
    private function updateVendorBankDetails($id, $request)
    {
        $data = $request->all();
